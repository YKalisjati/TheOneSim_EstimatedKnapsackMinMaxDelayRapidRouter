 ...
 * @author asus
 */
public class EstimateKnapsackTestRouter extends ActiveRouterForKnapsack {
     ...
     */
    public EstimateKnapsackTestRouter(Settings s) {
        super(s);
...
//        System.out.println(host);
     ...
     */
    protected EstimateKnapsackTestRouter(EstimateKnapsackTestRouter r) {
        super(r);
...
//            if (getHost().toString().startsWith("j") && otherHost.toString().startsWith("j")) {
//            if (String.valueOf(getHost().toString().charAt(0)).equals("j")) {
...
//            }
    ...
    private void doHostMapping(Connection con) {
        ...
        EstimateKnapsackTestRouter otherRouter = ((EstimateKnapsackTestRouter) otherHost.getRouter());
...
//            System.out.println("PESAN BARU "+m.getId()+" di buat oleh "+getHost()+" ukuran "+m.getSize()+" Tujuan "+m.getTo());
    ...
    public int receiveMessage(Message m, DTNHost from) {
        ...
        if (stat == 0) {
            ...
            ((EstimateKnapsackTestRouter) from.getRouter()).updateDelayTableEntry(m, host, delay, time);
        }
    ...
    private double getMarginalUtility(Message msg, DTNHost otherHost, DTNHost host) {
        final EstimateKnapsackTestRouter otherRouter = (EstimateKnapsackTestRouter) (otherHost.getRouter());
     ...
     * @return the current MU value
     */
    private double getMarginalUtility(Message msg, EstimateKnapsackTestRouter router, DTNHost host) {
    ...
    private double computeRemainingTime(Message msg) {
        ...
        if (this.delayTable.getDelayEntryByMessageId(msg.getId()) != null) {
            ...
            for (Map.Entry<DTNHost, Tuple<Double, Double>> entry : this.delayTable.getDelayEntryByMessageId(msg.getId()).getDelays()) {
                ...
                transferTime = ((EstimateKnapsackTestRouter) host.getRouter()).computeTransferTime(msg, msg.getTo()); //MXm(i)	with m element of [0...k]
...
//			Message m = t.getKey();
    ...
    private Tuple<Tuple<Message, Connection>, Double> tryOtherMessages() {
        ...
        for (Connection con : getConnections()) {
            ...
            EstimateKnapsackTestRouter otherRouter = (EstimateKnapsackTestRouter) other.getRouter();
    ...
    @Override
    public EstimateKnapsackTestRouter replicate() {
        return new EstimateKnapsackTestRouter(this);
    ...
    public void cekSyaratKnapsackSend(DTNHost thisHost, DTNHost otherHost) {
        if (getSyaratKnapsack(thisHost, otherHost)) {
            ...
            listMessage.addAll(this.getMessageCollection());
